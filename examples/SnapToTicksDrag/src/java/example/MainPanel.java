...
public final class MainPanel extends JPanel {
  private MainPanel() {
    ...
    JSlider slider1 = new SnapToTicksDragSlider(0, 100, 50);
    slider1.setBorder(BorderFactory.createTitledBorder("Custom SnapToTicks"));
    MouseWheelListener handler = e -> {
...
class SnapToTicksDragSlider extends JSlider {
  protected SnapToTicksDragSlider(int min, int max, int value) {
    super(min, max, value);
  }
  ...
  private void startSnapDrag(Point pt) {
    if (isRequestFocusEnabled()) {
      requestFocusInWindow();
    }
    setValueIsAdjusting(true);
    setValue(getSnappedValue(pt));
  }

  @Override protected void processMouseMotionEvent(MouseEvent e) {
    if (e.getID() == MouseEvent.MOUSE_DRAGGED && isSnapDragEvent(e)) {
  ...
  private boolean isSnapDragEvent(MouseEvent e) {
    return isEnabled() && getSnapToTicks() && getTickSpacing() > 0
        && getUI() instanceof BasicSliderUI && SwingUtilities.isLeftMouseButton(e);
  }

  private int getTickSpacing() {
    boolean hasMinorTick = getMinorTickSpacing() > 0;
    return hasMinorTick ? getMinorTickSpacing() : getMajorTickSpacing();
  }
  ...
  private int getSnappedValue(Point pt) {
    ...
    int snapped = min + Math.round((value - min) / (float) tickSpacing) * tickSpacing;
    return Math.max(min, Math.min(getMaximum(), snapped));
  }
}
