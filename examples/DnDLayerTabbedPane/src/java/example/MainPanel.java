...
class TabTransferHandler extends TransferHandler {
  ...
  @Override public int getSourceActions(JComponent c) {
    ...
    if (c instanceof DnDTabbedPane && ((DnDTabbedPane) c).getDragTabIndex() >= 0) {
      updateDragTabImage(createDragTabImage((DnDTabbedPane) c));
      action = MOVE;
    } else {
      action = NONE;
    }
...
class DropLocationLayerUI extends LayerUI<DnDTabbedPane> {
    ...
    if (c instanceof JLayer) {
      ...
      Optional.ofNullable(tabbedPane.getDropLocation())
          ...
          .ifPresent(loc -> {
            ...
            g2.fill(getLineRect(tabbedPane, loc));
            g2.dispose();
          });
  ...
  private static Rectangle getLineRect(JTabbedPane tabbedPane, DnDTabbedPane.DropLocation loc) {
    int index = loc.getIndex();
    int a = Math.min(index, 1); // index == 0 ? 0 : 1;
    Rectangle r = tabbedPane.getBoundsAt(Math.max(index - 1, 0));
    return DnDTabbedPane.isTopBottomTabPlacement(tabbedPane.getTabPlacement())
